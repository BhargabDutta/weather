...
function requestApi(city){
    api = `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=bdf229a70723e69abcbb41b631e52d27`;
    fetchData();
}

function onSuccess(position){
    const {latitude, longitude} = position.coords; // getting lat and lon of the user device from coords obj
    api = `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&units=metric&appid=bdf229a70723e69abcbb41b631e52d27`;
    fetchData();
}
...
function weatherDetails(info){
    if(info.cod == "404"){ // if user entered city name isn't valid
        ...
        if(id == 800){
            ...
            document.body.style.backgroundImage="url('icons/snowy.jpg')";
            wIcon.src = "icons/snow.png";
        }else if(id >= 701 && id <= 781){
            document.body.style.backgroundImage="url('icons/hazy.jpg')";
